(() => {
  ...
  // Derive a Short Authentication String (emoji + hex)
  ...
  class CryptoSession {
    ...
    async derive(sharedBindInfo) {
      ...
      const prefix = new Uint8Array(senderIdHash).slice(0, 6);
  ...
  // ---- UI wiring (boot after DOM is ready) ---------------------------------
  ...
  function boot() {
    ...
    const missing = Object.entries(ui)
      ...
      .map(([k]) => k);
    if (missing.length) {
      console.error('Missing DOM elements for IDs:', missing);
      return;
    }
